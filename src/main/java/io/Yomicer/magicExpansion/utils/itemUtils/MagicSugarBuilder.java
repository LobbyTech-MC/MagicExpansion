...
public class MagicSugarBuilder {
     ...
     */
    public static SlimefunItemStack getMagicSugar(int power) {
        if (power < 1 || power > 39) {
            throw new IllegalArgumentException("幂次必须在 1 到 39 之间");
        }

        String id = "MAGIC_EXPANSION_MAGIC_SUGAR_" + power;
        String displayName = "魔法糖*9" + toSuperscript(power);
        // 计算 9^power 的真实值
        BigInteger base = BigInteger.valueOf(9);
        BigInteger result = base.pow(power); // 计算 9^power
        String lore = "堆叠了" + result + "个魔法糖";

        return new SlimefunItemStack(
                id,
                doGlow(Material.SUGAR),
